...
namespace P2PNetwork
{
    ...
    public class TunDriveWinSDK : TunDriveSDK
    {
        ...
        public TunDriveWinSDK(IConfiguration configuration, ILogger<TunDriveSDK> logger) : base(configuration, logger)
        {
        }
        ...
        public static void Extract()
        {
            ...
            ZipArchive zipArchive = new ZipArchive(typeof(TunDriveService).Assembly.GetManifestResourceStream($"P2PNetwork.{(Environment.Is64BitOperatingSystem ? "amd64" : "i386")}.zip"), ZipArchiveMode.Read);
        ...
        public override bool ConnectionState(bool connection)
        {
            ...
            StartProcess("netsh", $"netsh interface ip set subinterface \"{DriveName}\" mtu=\"1400\"");
            return DeviceIoControl(FileStream.SafeFileHandle, CTL_CODE(FILE_DEVICE_UNKNOWN, TAP_WIN_IOCTL_SET_MEDIA_STATUS, METHOD_BUFFERED, FILE_ANY_ACCESS), cconfig, 4, cconfig, 4, ref Length, IntPtr.Zero);
        }
